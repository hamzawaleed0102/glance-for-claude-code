...
- **Title source precedence** (`Agent.applyState`): the title is claimed **once per conversation** — `applyState` only accepts an AI title while the source is still `'default'`. The first `update_state` with a usable title sets source to `'ai'` and locks it; `'ai'`, `'manual'`, and `'rename'` all reject further AI titles, so the card/terminal tab never renames mid-conversation even if Claude supplies a different title later. `/clear` and `/compact` (SessionStart source=`clear`/`compact`) call `resetCardState`, which resets the title back to `Glance` and the source to `'default'` so the next turn's `update_state` re-claims it. Manual renames and AI titles both get first-letter capitalization via `capitalizeFirstLetter`.
